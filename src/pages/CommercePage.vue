  ...
  async created() {
    ...
    for (let comercio of comercios) {
      const q = query(
        ...
        where("itemId", "==", comercio.id)
      );
      const querySnapshot = await getDocs(q);
      const ratings = querySnapshot.docs.map((doc) => doc.data().avaliacao);

      console.log(`Ratings for ${comercio.nome}:`, ratings);

      const averageRating =
        ratings.reduce((acc, rating) => acc + rating, 0) / ratings.length || 0;

      console.log(`Average rating for ${comercio.nome}:`, averageRating);

      comercio.rating = averageRating;
    }
